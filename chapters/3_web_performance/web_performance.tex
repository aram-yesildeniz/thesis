...
%TODO add Always with performance in mind ??
...
This process involves establishing a connection between a client and a server, which is discussed in section X, and the browser's task of converting the data received from the server into a readable, ready-to-use website, which is discussed in section X.
...
The front-end takes care of everything the user sees on the screen, such as rendering the UI.
The front-end consists of a client (browser) that requests data from the back-end.
The back-end, usually a web server, processes the client's requests and sends the response back to the front-end.
The data is transmitted over the network, which connects the front and back ends and consists of network components such as cables, routers, switches, and so on.

Figure X shows the three entities and their tasks during the website loading process.
What happens in the network phase is described in Section X.
The front-end's task of receiving data and transforming it into a website is covered in Section X.
...
How they affect performance is discussed in Section X.
Protocols and other mechanisms for establishing a reliable and secure connection between the front-end and the back-end are described in Section X.
They are described here because they are important to the general understanding of the website loading process and because web performance metrics measure how long they take, as described in Section X. 
...
As examined in the following, latency is the bottleneck of network performance, and not bandwidth.
