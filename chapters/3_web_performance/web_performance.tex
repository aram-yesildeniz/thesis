...
With the async attribute, the browser downloads the JS in the background while it continues parsing the HTML (figure \ref{figure:script_loading}).
...
As with async, scripts with the defer attribute allow the browser to download the script in parallel while it continues parsing the HTML (figure \ref{figure:script_loading}).
...
\caption[Script processing by attribute]{Script processing by attribute \footnote{taken from \url{https://html.spec.whatwg.org/multipage/scripting.html} [11.08.2021]}} %TODO fix this, is not shown
...
%TODO add somewhere that those specs are not implemented by all browsers --> can i use
...
%TODO move footnotes also to biblio ? e.g. mdn dom content loaded
\begin{center}
	\small
	\begin{longtable}{ | p{0.3\linewidth} | p{0.6\linewidth} | }
	\hline
...
\begin{table}[h]
	...
	\hline
	Navigation and Resource Timing
	& Used to calculate a variety of metrics. \\
	\hline
	User Timing
...
They are listed in table \ref{table:wpt_high_level_metrics}.
...
\begin{table}[h]
	...
	\label{table:wpt_high_level_metrics}
\end{table}
...
The grades calculated by WPT are listed in table \ref{table:wpt_grades}.
...
\begin{table}[h]
	...
	\medskip
	\caption{WPT Grades}
	\label{table:wpt_grades}
\end{table}


%TODO say that WPT exposes a lot more metrics, e.g. when downloading test result files. For example DOM Content Loaded, which is used in evaluation
...
%TODO use floating environment so that listing fits on one page
\begin{center}
...
%TODO show how they correlate with the other Navigation Timing Metrics !!
...
% copy this to textedit to see nice tabs

%Navigation Timing Metric						WPT					GA
%
%PLT: loadEventStart - navigationStart					Document Complete, Load Event Start	pageLoadTime
...
%fetchStart - navigationStart						...					redirectionTime
%domInteractive - navigationStart					...					domInteractiveTime
%DOM Content Loaded: domContentLoadedEventStart - navigationStart	domContentLoadedEventStart		domContentLoadedTime
...
There are several types of web performance metrics (table \ref{table:performance_metrics_conclusion}):
...
With Web and Core Web Vitals, Google defines its own set of user-centric performance metrics.
...
%TODO make new table:
% or do not use type column, instead use columns "page weight", "navigation timing", "visual and user-centric" and make a cross if its this type ?
...
%	\begin{tabular}{ | p{0.3\linewidth} | p{0.6\linewidth} | }
	\hline
	Type \cellcolor{lightgrey} & Metric \cellcolor{lightgrey} & Synthetic Monitoring \cellcolor{lightgrey} &  RUM \cellcolor{lightgrey} \\ 
	\hline
%	Page Weight & Bytes and requests for each resource type, such as \\
%	& HTML, CSS, JS, Fonts, Images, etc. \\
	Navigation Timing &TTFB & x & - \\
	& PLT & - & - \\
	& DNS/TCP/TLS Time & - & - \\
	& DOM Events/Timings & - & - \\
	& Request Time & - & - \\
	& Page Download Time & - & - \\
	\hline
	Visual and User-Centric Metrics & Start Render & - & - \\
	& AFT & - & - \\
	& Speed Index & - & - \\
	Web Vitals & FCP & - & - \\
	& FMP & - & - \\
...
% add e.g. measured by: browser, server. Technical stuff is measured in browser, everything else not in browser
% technical, UX, ...
