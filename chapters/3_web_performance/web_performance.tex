...
Google dropped First CPU Idle again, as "the difference [to TTI] isn't significant enough to justify maintaining two similar metrics". % https://web.dev/first-cpu-idle/
...
The blocking of the main thread is characterized through long tasks.
As described in section X. long tasks are tasks occupying the main thread for more than 50ms and they are defined in and exposed through the Long Tasks API.
...
% https://web.dev/tbt/
- lab metric
-  "helps quantify the severity of how non-interactive a page is prior to it becoming reliably interactive"


The synthetic monitoring metric TBT is related to RUM-only metric First Input Delay, which will be discussed below.
...
% [Transition]
The Web Vitals metrics described above are metrics types and questions...
Among those, three most important metrics...
