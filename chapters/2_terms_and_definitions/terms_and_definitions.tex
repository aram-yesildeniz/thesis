...
The browser continues to parse the HTML making requests and building the DOM, until it gets to the end, at which point it constructs the CSS object model. With the DOM and CSSOM complete, the browser builds the render tree, computing the styles for all the visible content. 
...
DOM is a tree structure and internal representation of the HTML for the browser. % cite https://developer.mozilla.org/en-US/docs/Web/Performance/How_browsers_work
DOM is specified in the living standard. % footnote https://dom.spec.whatwg.org/
...
The general process is translating Bytes -> characters -> tokens -> nodes -> object model. % cite https://developers.google.com/web/fundamentals/performance/critical-rendering-path/constructing-the-object-model
...
Therefore, HTML is considered to be a render blocking resource. % cite https://developers.google.com/web/fundamentals/performance/critical-rendering-path/render-blocking-css
As the parsing process blocks the rendering of the page.
...
Browser will begin parsing as soon as data is being received. %cite How Browsers Work https://developer.mozilla.org/en-US/docs/Web/Performance/How_browsers_work
...
How CSS is converted into CSSOM is part of next section.
...
A CSS is does not block parsing. % cite How Browsers Work https://developer.mozilla.org/en-US/docs/Web/Performance/How_browsers_work

% https://blog.logrocket.com/how-browser-rendering-works-behind-the-scenes-6782b0e8fb10/
- As soon as the parser reaches the line with <link rel="stylesheet" href="style.css">, a request is made to fetch the CSS file, style.css
- The DOM construction continues, and as soon as the CSS file returns with some content, the CSSOM construction begins


% from SO
When the parser sees a stylesheet to load, it issues the request to the server, and moves on. If there are other resources to load, these can all be fetched in parallel (subject to some HTTP restrictions). 
...
CSS is a render blocking resource, as rendering of the page can only happen when CSSOM and therefore CSS is available. % cite https://developers.google.com/web/fundamentals/performance/critical-rendering-path/render-blocking-css
...
% from SO
But only when the CSS resources are loaded and ready can the page be painted on the screen. 
That's render blocking, and because the fetches are in parallel, it's a less serious slow down.
...
The render tree is discussed in paragraph X
Before we need to discuss JS.
...
The JS can change the DOM % https://developer.mozilla.org/en-US/docs/Web/Performance/Critical_rendering_path
