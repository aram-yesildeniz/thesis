...
Figure X. does not include all captured values from PerformanceNavigationTiming interface and PerformanceResourceTiming interface, but only those important within the navigation process.
For document from different origins, the attributes in parenthesis may not be available.% cite https://www.w3.org/TR/navigation-timing-2/#processing-model

Table X gives an short explanation for each time stamp.
Those time stamps are then used to calculate performance metrics, as described in the next section.



%TODO add what we see when calling getEntriesByType ?
...
\begin{center}
	...
	\hline
	unloadEventStart & Set to 0 if there is no previous document. Otherwise, value is set to time when previous documents unload event gets fired by the browser. \\
	\hline
	unloadEventEnd & Set to 0 if there is no previous document.  . Otherwise, value is set to time when previous documents unload event gets fired by the browser.  \\
	\hline
...
\paragraph{Metrics calculated with Navigation and Resource Timing Level 2}

The exposed attributes from Navigation and Resource Timing are used to calculate common navigation timing metrics.
...
User Timing simplified and facilitates the usage of custom metrics (see section X.)
...
The goal of the API is to detect "long tasks", this are tasks that occupy main thread.
Long tasks are incicators of performance

% https://www.w3.org/TR/longtasks-1/

% Editors Draft: https://w3c.github.io/longtasks/
...
% Editors Draft: https://w3c.github.io/server-timing/
...
Paint Timing 1 is a First Public Working Draft from September 2017. % https://www.w3.org/TR/paint-timing/
...
%Paint or Render: "the browser has performed a "paint" (or "render") when it has converted the render tree to pixels on the screen. "
...
\small
	\begin{tabular}{ p{0.3\linewidth} | p{0.6\linewidth} }
	\hline
	Navigation Timing & Level 1: REC December 2021, Level 2: WD March 2021. Exposes timing information of the main document. \\ 
	\hline
	Resource Timing & Level 1: CR March 2017, Level 2: WD April 2021. Exposes timing information from requested resources and the resources they request.  \\  
	\hline
	Navigation and Resource Timing & Used to calculated a variety of metrics \\
	\hline
	User Timing & Level 2: REC February 2019.  Level 3: WD July 2021. Mark and measure custom timestamps.  \\  
	\hline
	Performance Timeline & REC December 2013,  Level 2: WD June 2021.  Subsumes gathered performance data from other specifications. \\  
	\hline
	Long Tasks & API 1: FPWD  September 2017  \\  
	\hline
	Server Timing & WD June 2021\\
	\hline
	Paint Timing & FPWD September 2017  \\  
	\hline
	\end{tabular}
\end{center}
