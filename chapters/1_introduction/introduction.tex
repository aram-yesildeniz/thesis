...
In literature, two main ideas and process flows are indicated, the first one being from the Web Analytics Association, and the second one derived from industries best practices and described in multiple papers.
...
Defining the right KPIs is a crucial step.
...
% [Comparison]
Comparing both proposed processes, it is evident that both 
WAA is more fine grained but also
where as Waisberg and Kaushiks idea is inherent cyclical.
...
As already mentioned in chapter X, two main data collection methods are established in web analytics: Log file analysis and Page Tagging.
